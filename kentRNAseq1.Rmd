...
To do this, we employ a her3 mutant created by CRISPER/Cas9 modification of the
her3 sequence that leads to a truncated 38 amino acid protein.

We also are using a morpholino targeted to the her3 mRNA to prevent protein
translation.
...
There are six groups with samples collected at either 24hpf or 72hpf. RNA from
the entire embryo was collected for all samples.

-   Control Wildtype WIK 24hpf samples (n = 3)
-   Her3 38aa knockout 24hpf samples (n = 3)
-   Her3 morpholino 24hpf samples (n = 3)
-   Her3 control mismatch morpholino 24hpf samples (n = 3)
-   Control Wildtype WIK 72hpf samples (n = 3)
-   Her3 38aa knockout 72hpf samples (n = 3)

ERCC spike-in:
24hpf WIK samples had ERCC 1, and all the others had ERCC 2 spike-in controls
added to facilitate evaluation of dynamic range and sensitivity of differential
expression analysis.

RNAseq data were generated on an Illumina NovaSeq to create 151bp paired-end
reads.
...
expression between the groups. For motif analysis we used Homer. For most
statistical analyses and plotting, we used R and ggplot2. To do pathway
analysis we used the Ingenuity Pathway Analysis software.
...
```{r plotFastqc, dependson='fastqc', eval=TRUE}
...
```{r, echo=FALSE, cache=FALSE}
knitr::read_chunk("sbatchCmds/countAligned.sh", labels = "countAlignedCode")
```
```{bash countAlignedCode, eval=FALSE, cache=FALSE}
```
```{bash countAligned, eval=TRUE}
...
```{r plotDECounts, dependson='edgeR', eval=TRUE}
combined_data <- NULL
...
I used the output from EdgeR to run IPA.
I output the canonical pathways and upstream regulators for each comparison

### Plots

```{r ipaPlots, eval=TRUE}
...
```{r pca, dependson=c('edgeR', 'ercc', 'plotDECounts'), eval=TRUE}
group_table <- tibble(samples = list.files(path = "output/aligned",
                                           pattern = ".bam$")) %>%
    mutate(samples = str_remove(samples, "output/aligned/") %>%
        str_remove(".bam")) %>%
...
### PCA of all samples except the Morpholino samples


pca <- prcomp(gene_counts[!grepl("-MO", rownames(gene_counts)), ])
...
plot_colors <- scales::brewer_pal(type = "qual", palette = "Set2")(4)
names(plot_colors) <- levels(prin_comps$group)

pca_no_mo <-
    ggplot(prin_comps, aes(x = PC1,
                           y = PC2,
                           color = group)) +
    geom_point(size = 3) +
    ggtitle("PCA Plot for all Samples Except Morpholino") +
    ggrepel::geom_text_repel(aes(label = samples %>%
                                    str_replace_all("_", " ") %>%
                                    str_replace("her3", "Her3")),
                             size = 3) +
...
ggsave("output/figures/PCA_no_mo.png",
       width = 8,
       height = 8)
...
```{r her3norm, dependson='edgeR', eval=TRUE}
...
```{r normScatterplots, dependson='edgeR', eval=TRUE}
...
```{r vennDiagrams, dependson='edgeR', eval=TRUE}
...
```{r listDeGenes, dependson='edgeR', eval=TRUE}
...
### Run Homer
```{r, echo=FALSE, cache=FALSE}
knitr::read_chunk("sbatchCmds/homer.sh", labels = "homerCode")
```
```{bash splitFastqCode, eval=FALSE, cache=FALSE}
```
```{bash runHomer, dependson='listDeGenes', eval=FALSE}
sbatch sbatchCmds/homer.sh
```

### Gather html files from homer outputs
```{bash gatherHomer, dependson='runHomer', eval=TRUE}
...
```{bash gatherHomerDenovo, dependson='runHomer', eval=TRUE}
