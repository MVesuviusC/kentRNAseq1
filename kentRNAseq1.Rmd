...
three 24hpf WIK samples had ERCC 1, and all the others had ERCC 2

include elife data for 24 hour and 72 hour
...
Filter DE gene list based on ERCC spike ins

IPA
-   24 hr - wik vs her3 mut
-   24 hr - wik her3 scrMO vs wik her3 MO
-   72 hr - wik vs her3 mut

change logFC cutoff to 1 to try
...
Plots for each comparison
    x = time point
    y = gene expr.
    group = gene - line connecting points
    color or facet by gene type (neural, muscle, etc)

Heatmaps

Table of total reads, % aligned, % in genes, # up/down regulated genes
...
# kept_df <- filterByExpr(gene_data, min.count = 0.1)

# gene_data <- gene_data[kept_df, keep.lib.sizes = FALSE] %>%
#     calcNormFactors(.) %>%
#     estimateDisp(.,  model.matrix(~ c(1, 1, 1, 2, 2, 2)))
...
# ggsave("output/figures/signifGeneVennDiagrams.png",
#     width = 15,
#     height = 25)
```
